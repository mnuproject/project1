...
class ClientInfo extends Thread {
	...
	public void run() {
		try {
			...
			System.out.println(GameServerRes.TAG + "메세지 통신 실패.");
		}
	} // end of run
	...
	private void protocolRESULT() {
		for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
			GameServerRes.vcClient.get(i).writer.println("END&*****RESERT*****");
			for (int j = 0; j < GameServerRes.vcClient.size(); j++) {
				GameServerRes.vcClient.get(i).writer.println(
						"END&[" + GameServerRes.vcClient.get(j).clientId + "] is SCORE => [" + GameServerRes.vcClient.get(j).score + "]");
			}
			GameServerRes.vcClient.get(i).writer.println("END&*****RESERT*****");
		}
	}
	...
	private void protocolEND(String[] parsReaderMsg) {
		if (isStart == true && isEnd == true) {
			protocolRESULT();
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).writer.println("END&GAME END");
				GameServerRes.vcClient.get(i).isEnd = false;
				GameServerRes.vcClient.get(i).isStart = false;
				GameServerRes.vcClient.get(i).isClickReadybtn = false;
				GameServerRes.vcClient.get(i).isRightAnswer = false;
				GameServerRes.vcClient.get(i).checkReady = 0;
				GameServerRes.vcClient.get(i).selectTurn = 1;
				GameServerRes.vcClient.get(i).score = 0;
			}
		}
	}
	...
	private void protocolSKIP(String[] parsReaderMsg) {
		if (parsReaderMsg[0].equals("SKIP")) {
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).isRightAnswer = true;
				GameServerRes.vcClient.get(i).writer.println("SERVER&Turn Skip.");
				GameServerRes.vcClient.get(i).writer.println("ANSWER&");
			}
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).selectProblem++;
			}
			if (selectProblem >= Word.problem.length) {
				for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
					GameServerRes.vcClient.get(i).selectProblem = 0;
				}
			}
			// 넘기기 버튼을 눌렀는데 다음턴이 없을 경우 게임 종료.
			if (selectTurn > GameServerRes.vcClient.size()) {
				for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
					GameServerRes.vcClient.get(i).isEnd = true;
				}
	...
	private void protocolEXIT(String[] parsReaderMsg) {
		if (parsReaderMsg[0].equals("EXIT")) {
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).writer.println("SERVER&[" + clientId + "] is Exit the room.");
			}
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				if (GameServerRes.vcClient.get(i).clientId.equals(parsReaderMsg[1])) {
					GameServerRes.vcClient.remove(i);
				}
			}
		}
	}
	...
	private void protocolREADY(String[] parsReaderMsg) {
		if (parsReaderMsg[0].equals("READY")) {
			...
			if (isClickReadybtn == true) {
				for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
					GameServerRes.vcClient.get(i).writer.println("SERVER&[" + clientId + "] is Set Ready.");
					GameServerRes.vcClient.get(i).checkReady++;
				}
				System.out.println("checkReady: " + checkReady);
			} else {

				for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
					GameServerRes.vcClient.get(i).writer.println("SERVER&[" + clientId + "] please wait a moment.");
					GameServerRes.vcClient.get(i).checkReady--;
				}
				System.out.println("checkReady: " + checkReady);
			}
	...
	private void protocolSTART() {
		if (checkReady == GameServerRes.vcClient.size() && isStart == false) {
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).writer.println("START&");
				GameServerRes.vcClient.get(i).writer.println("SERVER&GAME START.");
				GameServerRes.vcClient.get(i).isStart = true;
				GameServerRes.vcClient.get(i).isRightAnswer = true;
				System.out.println("GameServerRes.vcClient.get(" + i + ").isStart: " + GameServerRes.vcClient.get(i).isStart);
			}
		}
	...
	private void protocolTURN() {
		if (isStart == true && isRightAnswer == true) {
			...
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				if (GameServerRes.vcClient.get(i).userTurn == selectTurn) {
					for (int j = 0; j < GameServerRes.vcClient.size(); j++) {
						GameServerRes.vcClient.get(j).writer.println("NOTTURN&");
					}
					GameServerRes.vcClient.get(i).writer.println("TURN&");
					for (int j = 0; j < GameServerRes.vcClient.size(); j++) {
						GameServerRes.vcClient.get(j).writer.println("SERVER&[" + GameServerRes.vcClient.get(i).clientId + "] is Turn.");
					}
				}
			} // end of for
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).selectTurn++;
				GameServerRes.vcClient.get(i).isRightAnswer = false;
			}
	...
	private void protocolID(String[] parsReaderMsg) {
		if (parsReaderMsg[0].equals("ID")) {
			...
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).writer.println("SERVER&[" + clientId + "] is enter the room.");
				GameServerRes.vcClient.get(i).writer.println("ID&");
				GameServerRes.vcClient.get(i).userTurn = i + 1;
				GameServerRes.vcClient.get(i).score = 0;
				System.out.println("GameServerRes.vcClient.get(" + i + ").clientId: " + GameServerRes.vcClient.get(i).clientId);
				System.out.println("GameServerRes.vcClient.get(" + i + ").userTurn: " + GameServerRes.vcClient.get(i).userTurn);
				System.out.println("userTurn: " + userTurn);
			} // end of for
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				for (int j = 0; j < GameServerRes.vcClient.size(); j++) {
					GameServerRes.vcClient.get(i).writer.println("IDLIST&[" + GameServerRes.vcClient.get(j).clientId + "]");
				}
			}
		}
	}
	...
	private void protocolANSWER(String[] parsReaderMsg) {
		if (isStart == true && parsReaderMsg[1].equals(Word.problem[selectProblem])) {
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).writer
						.println("SERVER&[" + clientId + "] , [" + Word.problem[selectProblem] + "] is Right Answer");
				GameServerRes.vcClient.get(i).writer.println("ANSWER&");
				GameServerRes.vcClient.get(i).isRightAnswer = true;
				if (GameServerRes.vcClient.get(i).clientId.equals(clientId)) {
					GameServerRes.vcClient.get(i).score++;
					System.out.println("GameServerRes.vcClient.get( " + i + " ).score: " + GameServerRes.vcClient.get(i).score);
				}
			}
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).selectProblem++;
	...
	private void protocolChat(String[] parsReaderMsg) {
		if (parsReaderMsg[0].equals("CHAT") && parsReaderMsg.length > 1) {
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).writer.println("CHAT&[" + clientId + "]: " + parsReaderMsg[1]);
			}
			protocolANSWER(parsReaderMsg);
		} else if (parsReaderMsg[0].equals("CHAT")) {
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).writer.println("CHAT&[" + clientId + "] ");
			}
			for (int i = 0; i < GameServerRes.vcClient.size(); i++) {
				GameServerRes.vcClient.get(i).isRightAnswer = true;
			}
	...
	private void protocolDraw(String[] parsReaderMsg) {
		if (parsReaderMsg[0].equals("DRAW")) {
			for(int i = 0; i < GameServerRes.vcClient.size(); i++) {
				if (GameServerRes.vcClient.get(i) != this) {
					GameServerRes.vcClient.get(i).writer.println("DRAW&"+ parsReaderMsg[1]);
					}
				}
			}
		}
	...
	private void protocolColor(String[] parsReaderMsg) {
		if (parsReaderMsg[0].equals("COLOR")) {
			...
			for(int i = 0; i < GameServerRes.vcClient.size(); i++) {
				if (GameServerRes.vcClient.get(i) != this) {
					GameServerRes.vcClient.get(i).writer.println("COLOR&"+ parsReaderMsg[1]);
